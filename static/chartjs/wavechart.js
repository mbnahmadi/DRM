...
arrowImage2.src = '/static/arrow/arrow.png';

const removeImage2 = new Image();
removeImage2.src = '/static/arrow/remove.png';


const directionToAngle2 = {
    'N': 0,
    'NNE': 22.5,
    'NE': 45,
    'ENE': 67.5,
...
const wavedirAngles = waveDataDire.map(wavedir => directionToAngle2[wavedir] || 0);

const arrowPlugin2 = {
    id: 'waveDirectionArrows',
    afterDraw(chart) {
        ...
        if (!arrowImage2.complete) {
            return;
        }

        ctx.save();
        waveDataDire.forEach((wavedir, index) => { // استفاده از windir به‌جای wavedir
            ...
            if (['00', '03', '06', '09', '12', '15', '18', '21'].includes(hour)) {
                ...
                // انتخاب تصویر مناسب
                let imageToDraw;
                let angle = wavedirAngles[index];
                if (wavedir === '-') {
                    imageToDraw = arrowImage2; // اگر داده "-" باشد، تصویر remove.png
                    angle = 0; // بدون چرخش (چون احتمالاً نیازی به چرخش تصویر remove.png نیست)
                } else {
                    imageToDraw = arrowImage2; // در غیر این صورت، تصویر فلش اصلی
                }
...
new Chart(ctx2, {
    ...
    plugins: [backgroundPlugin2, chartAreaBorder2, arrowPlugin2]
});